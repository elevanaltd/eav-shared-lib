...
describe('useDropdownOptions', () => {
  ...
  it('should fetch dropdown options for specific field', async () => {
    ...
    const { result } = renderHook(() => useDropdownOptions('shot_type', mockSupabase), {
  ...
  it('should fetch all field types when no field specified', async () => {
    const mockOptions = [
      ...
      },
      {
        ...
        option_value: 'ext_building',
        option_label: 'EXT-BUILDING',
        sort_order: 1,
        created_at: '2025-01-01',
      },
    ]
    ...
    const { result } = renderHook(() => useDropdownOptions(undefined, mockSupabase), {
      wrapper: createWrapper(),
    })
